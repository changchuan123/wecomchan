...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('wecomchan_fixed.log'),
...
# 企业微信配置 - 使用正确的配置
SENDKEY = os.environ.get('SENDKEY', 'wecomchan_token')
...
def status():
    """状态检查接口"""
    return jsonify({
        "service": "wecomchan_server_fixed",
        "status": "running",
        "port": 5002,  # 使用不同端口避免冲突
        "timestamp": datetime.now().isoformat(),
        "config": {
            "sendkey_configured": SENDKEY != 'wecomchan_token',
            "wecom_cid_configured": WECOM_CID != 'ww5396d87e63595849',
            "wecom_aid_configured": WECOM_AID != '1000011'
        }
    })
...
if __name__ == '__main__':
    ...
    print("📝 使用方法:")
    print("   GET:  http://localhost:5002/?sendkey=你的key&msg=消息内容")
    ...
    print("📊 状态检查: http://localhost:5002/status")
