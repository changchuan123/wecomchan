...
def fix_trend_chart_category_shop_combo_v2():
    """修复趋势图品类和店铺同时筛选的问题"""
    ...
    # 1. 修复JavaScript数据结构，添加categoryShopData
    # 查找构建JavaScript数据的部分
    pattern = r'js_data = \{'
    replacement = '''js_data = {
            'dates': all_dates,
    ...
    # 2. 在填充单品数据之后添加categoryShopData的填充逻辑
    pattern = r'# 填充单品数据.*?js_data\[''productData''\]\[product\] = product_daily'
    replacement = '''# 填充单品数据 - 确保每个日期都有数据，没有销售的日期填充0
    ...
    # 保存修复后的文件
    with open('整体月报数据_fixed_v4.py', 'w', encoding='utf-8') as f:
        f.write(content)
    
    print("✅ 趋势图品类+店铺组合筛选修复完成（版本2）")
    print("📝 主要修复内容：")
    ...
    print("5. 修复了数据结构重复的问题")

if __name__ == "__main__":
    fix_trend_chart_category_shop_combo_v2() 