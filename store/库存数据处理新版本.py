...
"""
库存数据处理脚本 - 新版本
功能：从wdt和Date数据库获取库存数据，按仓库类型聚合，生成报告并推送到企业微信
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('库存数据处理新版本.log', encoding='utf-8'),
...
class InventoryProcessor:
    ...
    def send_wechat_message(self, message: str) -> bool:
        """发送企业微信消息"""
        # 检查配置是否完整
        if not all([WECOM_CONFIG['corpid'], WECOM_CONFIG['corpsecret'], WECOM_CONFIG['agentid']]):
            logger.warning("企业微信配置不完整，跳过消息发送")
            logger.info("请在脚本中填写企业微信配置信息")
            return False
...
🏪 仓库分布:"""
    ...
    def run(self):
        ...
        try:
            ...
            # 发送企业微信消息
            self.send_wechat_message(summary_message)
